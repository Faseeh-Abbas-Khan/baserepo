...
import CacheImage from './CachedImage/CachedImage';

interface cacheImageProp {
  url: string;
  id: string;
}
...
export interface AutoImageProps extends ImageProps {
  ...
  maxHeight?: number;
  /**
   * Cached Images (Default false)
   */
  cacheImage?: cacheImageProp;
}
...
export function AutoImage(props: AutoImageProps) {
  const {maxWidth, maxHeight, cacheImage, ...ImageProps} = props;
  ...
  return (
    <>
      {cacheImage ? (
        ...
        <Image {...ImageProps} style={[{width, height}, props.style]} />
      )}
    </>
  );
}
